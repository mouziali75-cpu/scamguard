...
(function() {
  ...
  const isTrustedGlobal = matches(trustedDomains) || matches(getAdminDomains('trusted'));
  ...
  function openAdminPanel() {
    const modal = showModal(`
      ...
      <select id="sg-admin-cat" style="width:100%;box-sizing:border-box;padding:10px;border-radius:8px;
        ...
        <option value="trusted" style="color:black;">✅ Trusted Domain</option>
      </select>
  ...
  function refreshListsFromGithub() {
    return fetch(RAW_SCRIPT_URL + '?t=' + Date.now())
      ...
      .then(text => {
        ...
        const remoteTrusted = extractArrayFromSource(text, 'trustedDomains');
        try {
          ...
          localStorage.setItem('sg_admin_trusted', JSON.stringify(remoteTrusted));
        } catch (e) {}
        return {
